...
2. **Ambiente di sviluppo.** Guidami a installare/verificare: Homebrew, Python 3, LibreOffice, font Liberation. Poi virtualenv del progetto e dipendenze base (FastAPI, uvicorn, docxtpl, SDK OpenAI, pytest, PyMuPDF/pdfplumber). Un comando alla volta, spiegato in una riga.
3. **Cartella dati.** Crea `~/Gestionale/` con la struttura di `04_ARCHITETTURA.md` §2 (`db.sqlite` vuoto, `documenti/`, `templates/`, `backup/`, `logs/`); documenta in `README.md` che lì vivono tutti i dati.
...
6. **Bozza backup.** Crea lo script `backup.sh` che usa `sqlite3 .backup` (MAI copia diretta dei file del database aperto) e copia il risultato in una destinazione offsite che scegliamo insieme (disco esterno o cloud cifrato). Aggiungi le istruzioni del test di ripristino su una copia di prova. Niente schedulazione automatica in questo sprint.
...
- [ ] Lancio `backup.sh` e trovo il file di backup nella destinazione; seguendo le istruzioni, ripristino su una copia di prova e la copia si apre.
